...
public abstract class AbstractDawnbreaker extends SwordItem implements GeoItem {
    ...
    public static void dawnbreakerEvent(LivingEntity target, LivingEntity attacker, ItemStack stack) {
        if (!attacker.method_48926().isClient && attacker instanceof ServerPlayerEntity) {
            //Replaces old DAWNBREAKER_PACKET call
            ParticleHandler.particleSphere(attacker.method_48926(), 1000, target.getX(), target.getEyeY() - .25f, target.getZ(), ParticleTypes.FLAME, 1f);
            ParticleHandler.particleOutburstMap(attacker.method_48926(), 200, target.getX(), target.getY(), target.getZ(), ParticleEvents.DAWNBREAKER_MAP, 1f);
        }
        target.method_48926().playSound(null, target.getBlockPos(), SoundRegistry.DAWNBREAKER_EVENT, SoundCategory.HOSTILE, 2f, 1f);
        Box aoe = target.getBoundingBox().expand(10);
        List<Entity> entities = attacker.method_48926().getOtherEntities(target, aoe);
            ...
            if (entity instanceof LivingEntity targetHit) {
                if (targetHit.isUndead() || bl) {
                    if (!targetHit.equals(attacker)) {
                        ...
                        targetHit.damage(attacker.method_48926().getDamageSources().explosion(null, attacker), ConfigConstructor.dawnbreaker_ability_damage + 5 * EnchantmentHelper.getLevel(Enchantments.FIRE_ASPECT, stack));
