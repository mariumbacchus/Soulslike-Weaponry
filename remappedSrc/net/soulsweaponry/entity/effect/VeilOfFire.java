...
public class VeilOfFire extends StatusEffect {
    ...
    public void applyUpdateEffect(LivingEntity entity, int amplifier) {
        for (LivingEntity target : entity.method_48926().getNonSpectatingEntities(LivingEntity.class, entity.getBoundingBox().expand(1.5D))) {
            target.damage(entity.getDamageSources().inFire(), 2f + amplifier);
            target.setOnFireFor(2 + amplifier);
        }
        if (entity.method_48926().isClient) {
            for (int i = 0; i < 50; i++) {
                entity.method_48926().addParticle(ParticleTypes.FLAME, entity.getParticleX(1D), entity.getBodyY(0.5) + entity.getRandom().nextDouble() * 2 - 1D, entity.getParticleZ(1D), 0, 0, 0);
            }
        }
