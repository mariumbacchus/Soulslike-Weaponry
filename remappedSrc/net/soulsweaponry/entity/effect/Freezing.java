...
public class Freezing extends StatusEffect {
    ...
    public void applyUpdateEffect(LivingEntity entity, int amplifier) {
        ...
        if (!entity.method_48926().isClient) {
            ParticleHandler.singleParticle(entity.method_48926(), ParticleTypes.SNOWFLAKE, entity.getParticleX(0.5D), entity.getRandomBodyY(), entity.getParticleZ(0.5D), 0, 0, 0);
        }
        if (entity.isDead()) {
            if (entity instanceof IAnimatedDeath animated) {
                if (animated.getDeathTicks() < 2) {
                    LeviathanAxe.iceExplosion(entity.method_48926(), entity.getBlockPos(), entity.getAttacker(), amplifier);
                }
            }
