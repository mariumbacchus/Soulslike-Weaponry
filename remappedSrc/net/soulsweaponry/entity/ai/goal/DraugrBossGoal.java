...
public class DraugrBossGoal extends MeleeAttackGoal {
    ...
    public boolean applyDamage(LivingEntity target, float baseDamage) {
        ...
        return target.damage(this.boss.method_48926().getDamageSources().mobAttack(this.boss), modified);
    }
    ...
    private void leapAttack(LivingEntity target, float damage, boolean stunTarget) {
        ...
        if (attackStatus == 26) {
            this.boss.method_48926().playSound(null, target.getBlockPos(), SoundEvents.ENTITY_ZOMBIE_BREAK_WOODEN_DOOR, SoundCategory.HOSTILE, 1f, 1f);
            if (this.isInMeleeRange(target)) {
                if (this.applyDamage(target, damage)) {
                    if (stunTarget) {
                    ...
                    if (!this.boss.method_48926().isClient) {
                        ParticleHandler.particleSphereList(this.boss.method_48926(), 10, target.getX(), target.getY(), target.getZ(), ParticleEvents.DARK_EXPLOSION_LIST, 1f);
                    }
                }
            }
        }
    ...
    private void parry(LivingEntity target) {
        ...
        if (attackStatus == 8 && this.isInMeleeRange(target)) {
            if (!target.hasStatusEffect(EffectRegistry.POSTURE_BREAK)) {
                this.boss.method_48926().playSound(null, target.getBlockPos(), SoundRegistry.POSTURE_BREAK_EVENT, SoundCategory.HOSTILE, .5f, 1f);
            }
    ...
    private void aoe(int maxTicks, int frame, float damage, float knockback, StatusEffect[] effects, double boxSize, boolean shieldUpWhenDone) {
        this.attackStatus++;
        this.boss.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 5, 20, false, true));
        if (effects.length > 0 && (attackStatus == 12 || attackStatus == 20)) this.boss.method_48926().playSound(null, this.boss.getBlockPos(), SoundRegistry.SWORD_HIT_SHIELD_EVENT, SoundCategory.HOSTILE, 1f, 1f);
        if (attackStatus == frame) {
            if (damage > 0) {
                if (!this.boss.method_48926().isClient) {
                    ParticleHandler.particleOutburstMap(this.boss.method_48926(), 300, this.boss.getX(), this.boss.getY(), this.boss.getZ(), ParticleEvents.OBLITERATE_MAP, 1f);
                }
            ...
            for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, this.boss.getBoundingBox().expand(boxSize))) {
    ...
    private void heavyBlow(LivingEntity target) {
        ...
        if (attackStatus == 24 && pos != null && isPosNotNullish(pos)) {
            this.boss.method_48926().playSound(null, pos, SoundEvents.ENTITY_WITHER_BREAK_BLOCK, SoundCategory.HOSTILE, 1f, 1f);
            if (!this.boss.method_48926().isClient) {
                ParticleHandler.particleSphereList(this.boss.method_48926(), 100, pos.getX(), pos.getY(), pos.getZ(), ParticleEvents.DARK_EXPLOSION_LIST, 1f);
            }
            for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, new Box(pos).expand(1D))) {
                if (entity instanceof LivingEntity living) {
