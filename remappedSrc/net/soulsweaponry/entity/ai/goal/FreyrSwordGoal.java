...
public class FreyrSwordGoal extends Goal {
    ...
    public void tick() {
        LivingEntity target = this.entity.getTarget();
        if (target != null) {
            this.attackTarget(target, this.entity.method_48926());
        }
    }
    ...
    private void attackTarget(LivingEntity target, World world) {
        ...
        for (double[] hitFrame : this.hitFrames) {
            if (this.attackTicks == hitFrame[0]) {
                ...
                if (target.damage(CustomDamageSource.create(this.entity.method_48926(), CustomDamageSource.FREYR_SWORD, this.entity, this.entity.getOwner()), (float) (this.getAttackDamage(target) * hitFrame[1]))) {
                    int fire;
                    if ((fire = EnchantmentHelper.getLevel(Enchantments.FIRE_ASPECT, this.entity.asItemStack())) > 0) {
                        target.setOnFireFor(fire * 4);
                    }
                    if (!world.isClient) {
                        ParticleHandler.singleParticle(this.entity.method_48926(), ParticleTypes.SWEEP_ATTACK, target.getX(), target.getEyeY(), target.getZ(), 0, 0, 0);
                    } else {
