...
public class DayStalkerGoal extends MeleeAttackGoal {
    ...
    */
    ...
    private void tickPartnerSwitch() {
        ...
        if (!this.boss.method_48926().isClient && (partner = this.boss.getPartner((ServerWorld) this.boss.method_48926())) != null) {
            if (timer == 0) {
    ...
    private boolean damageTarget(LivingEntity target, float damage) {
        ...
        if (target.damage(this.boss.method_48926().getDamageSources().mobAttack(this.boss), this.getModifiedDamage(damage))) {
    ...
    private void airCombustion(LivingEntity target) {
        ...
        if (this.attackStatus >= 16 && this.attackStatus <= 25 && this.attackStatus % 4 == 0) {
            this.boss.method_48926().playSound(null, this.boss.getTargetPos(), SoundEvents.BLOCK_FIRE_AMBIENT, SoundCategory.HOSTILE, 1f, 1f);
            this.boss.method_48926().playSound(null, this.boss.getTargetPos(), SoundEvents.ENTITY_ZOMBIE_VILLAGER_CONVERTED, SoundCategory.HOSTILE, 1f, 1f);
        }
        if (this.attackStatus >= 20 && this.attackStatus <= 36) {
            if (!this.boss.method_48926().isClient) {
                ...
                ParticleHandler.particleOutburst(this.boss.method_48926(), 150, center.getX(), center.getY(), center.getZ(), ParticleTypes.FLAME, new Vec3d(10 ,10 ,10), 1f);
            }
        }
        if (this.attackStatus == 37) {
            if (!this.boss.method_48926().isClient) {
                this.boss.method_48926().playSound(null, this.boss.getTargetPos(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.HOSTILE, 1f, 1f);
                Vec3d pos = Vec3d.ofCenter(this.boss.getTargetPos());
                ParticleEvents.airCombustionEvent(this.boss.method_48926(), pos.getX(), pos.getY(), pos.getZ());
                for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, new Box(this.boss.getTargetPos()).expand(this.boss.isPhaseTwo() ? 2D : 1D))) {
        ...
        if (this.attackStatus == 29) {
            ...
            if (this.boss.isPhaseTwo()) {
                ...
                for (int i = 0; i < 16; ++i) {
                    ...
                    if (p != null) {
                        this.boss.method_48926().setBlockState(p, Blocks.FIRE.getDefaultState());
                    }
                }
            }
        }
    ...
    private void shootProjectile(LivingEntity target, ProjectileEntity projectile, SoundEvent sound) {
        ...
        this.boss.method_48926().playSound(null, this.boss.getBlockPos(), sound, SoundCategory.HOSTILE, 1f, 1f);
        if (target.getBlockPos() != null) this.boss.method_48926().playSound(null, target.getBlockPos(), sound, SoundCategory.HOSTILE, 1f, 1f);
    ...
    private BlockPos conjureFlames(double x, double z, double maxY, double y) {
        ...
        do {
            ...
            BlockPos blockPos2;
            if (!this.boss.method_48926().getBlockState(blockPos2 = blockPos.down()).isSideSolidFullSquare(this.boss.method_48926(), blockPos2, Direction.UP)) continue;
            if (!this.boss.method_48926().isAir(blockPos) && !(voxelShape = this.boss.method_48926().getBlockState(blockPos).getCollisionShape(this.boss.method_48926(), blockPos)).isEmpty()) {
        ...
        if (this.attackStatus == 1) {
            ...
            while (i < (phase2 ? 32 : 16)) {
                ...
                for (BlockPos listPos : list) {
                    if (listPos != pos) {
                        FlamePillar pillar = new FlamePillar(EntityRegistry.FLAME_PILLAR, this.boss.method_48926());
                        pillar.setDamage(this.getModifiedDamage(48f));
                        pillar.setPos(x, y, z);
                        pillar.setOwner(this.boss);
                        pillar.setWarmup(i * 2);
                        this.boss.method_48926().spawnEntity(pillar);
                        i++;
                    }
                }
            }
    ...
    private void dawnbreaker(LivingEntity target) {
        ...
        if (this.attackStatus == 11 || this.attackStatus == 21) {
            ...
            if (this.isInMeleeRange(target)) {
                ...
                if (!this.boss.method_48926().isClient) {
                    ParticleHandler.singleParticle(this.boss.method_48926(), ParticleTypes.SWEEP_ATTACK, target.getX(), target.getEyeY(), target.getZ(), 0, 0, 0);
                }
            }
            if (this.boss.isPhaseTwo()) {
    ...
    private void shootSunlight(LivingEntity target, int typeIndex, float damage, MoonlightProjectile.RotationState rotationState) {
        if (typeIndex == 0) {
            MoonlightProjectile projectile = new MoonlightProjectile(EntityRegistry.SUNLIGHT_PROJECTILE_SMALL, this.boss.method_48926(), this.boss);
            ...
            MoonlightProjectile projectile = new MoonlightProjectile(EntityRegistry.SUNLIGHT_PROJECTILE_BIG, this.boss.method_48926(), this.boss);
            ...
            MoonlightProjectile projectile = new MoonlightProjectile(EntityRegistry.VERTICAL_SUNLIGHT_PROJECTILE, this.boss.method_48926(), this.boss);
    ...
    private void flamethrower(LivingEntity target, double distance) {
        ...
        if (this.attackStatus >= 20 && this.attackStatus < 65 && target.getBlockPos() != null) {
            ...
            if (this.attackStatus % 4 == 0) {
                ...
                for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, box)) {
    ...
    private void aoeMelee(LivingEntity target, double distanceToTarget, double expansion, float damage) {
        ...
        for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, new Box(pos).expand(expansion))) {
            if (entity instanceof LivingEntity living) {
    ...
    private void conflagration(LivingEntity target) {
        ...
        if (!phase2) {
            if (this.attackStatus >= 20 && this.attackStatus <= 40 && this.attackStatus % 4 == 0) {
                ...
                UntargetableFireball fireballEntity = new UntargetableFireball(this.boss.method_48926(), this.boss, f, g, h, 2);
                fireballEntity.setPosition(this.boss.getX(), this.boss.getEyeY() + 1D, fireballEntity.getZ());
                this.boss.method_48926().spawnEntity(fireballEntity);
            }
        } else {
            this.boss.getNavigation().stop();
            if (this.attackStatus == 15) {
                GrowingFireball fireballEntity = new GrowingFireball(this.boss.method_48926(), this.boss);
    ...
    private void blazeBarrage(LivingEntity target) {
        ...
        for (int i = 0; i < (this.boss.isPhaseTwo() ? 2 : 1); i++) {
            ...
            SmallFireballEntity fireball = new SmallFireballEntity(this.boss.method_48926(), this.boss.getX(), this.boss.getY(), this.boss.getZ(), x, y, z);
            ...
            this.boss.method_48926().spawnEntity(fireball);
            if (this.attackStatus % 4 == 0) this.boss.method_48926().playSound(null, this.boss.getBlockPos(), SoundEvents.ENTITY_BLAZE_SHOOT, SoundCategory.HOSTILE, 1f, 1f);
        }
    ...
    public void aoe(double expansion, float damage, float knockback) {
        for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, this.boss.getBoundingBox().expand(expansion))) {
    ...
    private void radiance() {
        ...
        if (this.attackStatus == 80) {
            CustomDeathHandler.deathExplosionEvent(this.boss.method_48926(), this.boss.getPos(), SoundRegistry.DAWNBREAKER_EVENT, ParticleTypes.LARGE_SMOKE, ParticleTypes.FLAME);
            this.aoe(4D, 60f, 4f);
        }
    ...
    private void warmth() {
        ...
        if (this.attackStatus == 84) {
            this.boss.method_48926().playSound(null, this.boss.getBlockPos(), SoundEvents.BLOCK_LAVA_EXTINGUISH, SoundCategory.HOSTILE, 1f, 1f);
            for (int i = 0; i < 4; i++) {
                WarmthEntity entity = new WarmthEntity(EntityRegistry.WARMTH_ENTITY, this.boss.method_48926());
    ...
    private void overheat(LivingEntity target) {
        ...
        this.boss.getNavigation().stop();
        if (this.attackStatus == 1) {
            this.boss.method_48926().playSound(null, this.boss.getBlockPos(), SoundRegistry.OVERHEAT_CHARGE_EVENT, SoundCategory.HOSTILE, 1f, 1f);
            this.boss.method_48926().playSound(null, target.getBlockPos(), SoundRegistry.OVERHEAT_CHARGE_EVENT, SoundCategory.HOSTILE, 1f, 1f);
        }
        ...
        if (this.attackStatus == 67 && this.attackRotation != 0 && this.targetY != 0 && this.targetMaxY != 0) {
            ...
            for (int i = 0; i <= length; ++i) {
                ...
                if (pos != null) {
                    if (!this.boss.method_48926().isClient) {
                        ParticleHandler.particleOutburstMap(this.boss.method_48926(), 200, pos.getX(), pos.getY(), pos.getZ(), ParticleEvents.FLAME_RUPTURE_MAP, 1f);
                    }
                    this.boss.method_48926().playSound(null, pos, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.HOSTILE, 1f, 1f);
                }
            }
            ...
            if (lastPos != null) {
                for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, new Box(this.boss.getPos().add(0, 2, 0), lastPos.toCenterPos()).expand(1D))) {
                    if (entity instanceof LivingEntity living) {
    ...
    private void inferno() {
        ...
        if (this.attackStatus >= 70 && this.attackStatus <= 100) {
            if (this.boss.isOnGround() && !this.hasExploded) {
                this.boss.method_48926().createExplosion(this.boss, this.boss.getX(), this.boss.getY(), this.boss.getZ(), 6f, World.ExplosionSourceType.NONE);
                this.hasExploded = true;
            }
        }
        if (this.attackStatus == 125) {
            this.boss.method_48926().createExplosion(this.boss, this.boss.getX(), this.boss.getY(), this.boss.getZ(), 8f, World.ExplosionSourceType.NONE);
        }
        this.checkAndReset(30, 0);
    }
    ...
    private void skyHigh() {
        ...
        if (this.attackStatus >= 146 && this.attackStatus <= 190) {
            if (this.boss.isOnGround() && !this.hasExploded) {
                ...
                for (Entity targets : this.boss.method_48926().getOtherEntities(this.boss, box)) {
                    if (targets instanceof LivingEntity livingEntity) {
                        this.damageTarget(livingEntity, power);
                    }
                }
                this.boss.method_48926().playSound(null, this.boss.getBlockPos(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.PLAYERS, 1f, 1f);
                float pDistance = this.fallDistance >= 10 ? this.fallDistance/10 : 1;
                if (!this.boss.method_48926().isClient) {
                    ParticleHandler.particleOutburstMap(this.boss.method_48926(), MathHelper.floor(200 * pDistance), this.boss.getX(), this.boss.getY(), this.boss.getZ(), ParticleEvents.DEFAULT_GRAND_SKYFALL_MAP, pDistance);
                }
                this.hasExploded = true;
            }
        }
    ...
    private void flamesReach(LivingEntity target, double distance) {
        ...
        if (this.attackStatus == 43) {
            ...
            if (!this.boss.method_48926().isClient) {
                ParticleHandler.particleOutburstMap(this.boss.method_48926(), 150, pos.getX(), pos.getY(), pos.getZ(), ParticleEvents.DARKIN_BLADE_SLAM_MAP, 1f);
            }
            this.playSound(blockPos, SoundEvents.ENTITY_GENERIC_EXPLODE, 1f, 1f);
            for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, new Box(blockPos).expand(3D))) {
                if (entity instanceof LivingEntity living) {
    ...
    private void playSound(@Nullable  BlockPos pos, SoundEvent sound, float volume, float pitch) {
        ...
        this.boss.method_48926().playSound(null, pos, sound, SoundCategory.HOSTILE, volume, pitch);
    }

    private Vec3d getAirBlockPos(Vec3d start, int counter) {
        if (this.boss.method_48926().getBlockState(BlockPos.ofFloored(start)).isAir() || counter >= 5) {
            return start;
        } else {
