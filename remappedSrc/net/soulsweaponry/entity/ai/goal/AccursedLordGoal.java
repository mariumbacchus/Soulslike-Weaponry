...
public class AccursedLordGoal extends Goal {
    ...
    private void handSlamLava() {
        ...
        if (this.attackStatus == 13 || this.attackStatus == 18 || this.attackStatus == 24) {
            ...
            if (!this.boss.method_48926().isClient) {
                ParticleHandler.particleOutburstMap(this.boss.method_48926(), 200, this.boss.getX(), this.boss.getY(), this.boss.getZ(), ParticleEvents.DARKIN_BLADE_SLAM_MAP, 1f);
            }
    ...
    private void summonLava(int r) {
        for (int theta = 0; theta < 360; theta += 15) {
            ...
            if (this.boss.method_48926().getBlockState(pos).isAir()) {
                this.boss.method_48926().setBlockState(pos, Blocks.LAVA.getDefaultState());
        ...
        if (this.attackStatus >= 7 && this.attackStatus <= 40) {
            ...
            for (Entity nearbyEntity : nearbyEntities) {
                if (nearbyEntity instanceof LivingEntity closestTarget) {
                    ...
                    this.damageTarget(closestTarget, this.boss.method_48926().getDamageSources().mobAttack(this.boss), 10f);
        ...
        if (this.attackStatus >= 16 && this.attackStatus <= 18) {
            ...
            for (Entity nearbyEntity : nearbyEntities) {
                if (nearbyEntity instanceof LivingEntity closestTarget) {
                    ...
                    this.damageTarget(closestTarget, this.boss.method_48926().getDamageSources().mobAttack(this.boss), 50f);
                }
            }
            if (!this.boss.method_48926().isClient) {
                ParticleHandler.particleSphere(this.boss.method_48926(), 1000, this.boss.getX(), this.boss.getY() + .4f, this.boss.getZ(), ParticleTypes.FLAME, 1f);
                ParticleHandler.particleOutburstMap(this.boss.method_48926(), 200, this.boss.getX(), this.boss.getY() + .1f, this.boss.getZ(), ParticleEvents.DAWNBREAKER_MAP, 1f);
            }
        }
    ...
    private void pullAttack(LivingEntity target) {
        ...
        if (this.attackStatus == 20) {
            ...
            this.damageTarget(target, this.boss.method_48926().getDamageSources().mobAttack(this.boss), 5f);
    ...
    private void swordSlam() {
        ...
        if (this.attackStatus == 17) {
            if (!this.boss.method_48926().isClient) {
                ParticleHandler.particleOutburstMap(this.boss.method_48926(), 200, this.attackPos.getX(), this.attackPos.getY(), this.attackPos.getZ(), ParticleEvents.DARKIN_BLADE_SLAM_MAP, 1f);
            }
            for (Entity entity : entities) {
                if (entity instanceof LivingEntity) {
                    this.damageTarget((LivingEntity) entity, CustomDamageSource.create(this.boss.method_48926(), CustomDamageSource.OBLITERATED, this.boss), 30f);
                    entity.setVelocity(entity.getVelocity().x, .3f, entity.getVelocity().z);
                }
            }
            this.boss.method_48926().playSound(null, this.attackPos, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.HOSTILE, 1f, 1f);
        }
        if (this.attackStatus >= 20) {
            this.resetAttackCooldown(.5f);
            this.cordsRegistered = false;
    ...
    private void projectileBarrage(LivingEntity target, double distanceToEntity, BarrageProjectiles entity) {
        ...
        if (this.attackStatus % 2 == 0 && this.attackStatus > 8) {
            ...
            for(int i = 0; i < fireSprayCount; ++i) {
                new SmallFireballEntity(this.boss.method_48926(), this.boss, e + this.boss.getRandom().nextGaussian() * h, f, g + this.boss.getRandom().nextGaussian() * h);
                ProjectileEntity projectile = switch (entity) {
                    case FIREBALLS ->
                            new SmallFireballEntity(this.boss.method_48926(), this.boss, e + this.boss.getRandom().nextGaussian() * h, f, g + this.boss.getRandom().nextGaussian() * h);
                    case WITHERBALLS ->
                            new ShadowOrb(this.boss.method_48926(), this.boss, e + this.boss.getRandom().nextGaussian() * h, f, g + this.boss.getRandom().nextGaussian() * h,
