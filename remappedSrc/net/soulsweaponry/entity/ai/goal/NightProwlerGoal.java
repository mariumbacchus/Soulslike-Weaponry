...
public class NightProwlerGoal extends MeleeAttackGoal {
    ...
    private void trinity() {
        ...
        DayStalker partner;
        if (this.boss.isFlying() && this.attackStatus == stopFlying && !this.boss.method_48926().isClient && (partner = this.boss.getPartner((ServerWorld) this.boss.method_48926())) != null) {
            this.boss.setFlying(false);
        ...
        if (!this.hasExploded && this.attackStatus >= min && this.attackStatus <= max && this.boss.isOnGround()) {
            ...
            ParticleHandler.flashParticle(this.boss.method_48926(), target.getX(), target.getY(), target.getZ(), new ParticleHandler.RGB(142, 107, 1), 10f);
            ParticleHandler.flashParticle(this.boss.method_48926(), target.getX(), target.getY(), target.getZ(), new ParticleHandler.RGB(72, 0, 140), 2f);
            ParticleHandler.particleOutburstMap(this.boss.method_48926(), 300, pos.getX(), pos.getY(), pos.getZ(), ParticleEvents.OBLITERATE_MAP, 1f);
    ...
    private void trinityShockwave() {
        ...
        for (int theta = 0; theta < 360; theta += 15) {
            ...
            NightWaveEntity wave = new NightWaveEntity(EntityRegistry.NIGHT_WAVE, this.boss.method_48926());
            ...
            this.boss.method_48926().spawnEntity(wave);
        }
    }
    ...
    private void nightsEmbrace() {
        ...
        if (!this.boss.method_48926().isClient && this.attackStatus == trigger) {
            DayStalker partner = phase2 ? null : this.boss.getPartner((ServerWorld) this.boss.method_48926());
            ...
            for (int theta = 0; theta < 360; theta += 360/amount) {
                ...
                Remnant entity;
                if (phase2) {
                    entity = new Forlorn(EntityRegistry.FORLORN, this.boss.method_48926());
                    HashMap<Enchantment, Integer> map = new HashMap<>();
                    map.put(Enchantments.PROTECTION, 2);
                    map.put(Enchantments.VANISHING_CURSE, 1);
                    Forlorn.initEquip(entity, map);
                } else {
                    entity = new SoulReaperGhost(EntityRegistry.SOUL_REAPER_GHOST, this.boss.method_48926());
                }
                pos = this.getNonAirPos(pos, this.boss.method_48926());
                entity.setPos(pos.getX(), pos.getY() + .1f, pos.getZ());
                ParticleHandler.particleOutburstMap(this.boss.method_48926(), 100, pos.getX(), pos.getY(), pos.getZ(), ParticleEvents.CONJURE_ENTITY_MAP, 1f);
                DeathSpiralEntity spiral = new DeathSpiralEntity(this.boss.method_48926(), entity.getPos(), 1f);
                spiral.setPosition(entity.getPos());
                this.boss.method_48926().spawnEntity(spiral);
    ...
    private boolean isSummonsAlive() {
        ...
        for (int i = 0; i < list.length; i++) {
            if (this.boss.method_48926().getEntityById(list[i]) != null) {
                arr[i] = list[i];
            }
        }
    ...
    private void conjureFangs(double x, double z, double maxY, double y, float yaw, int warmup) {
        ...
        if (bl) {
            if (this.boss.isPhaseTwo()) {
                NightsEdge edge = new NightsEdge(EntityRegistry.NIGHTS_EDGE, this.boss.method_48926());
                ...
                this.boss.method_48926().spawnEntity(edge);
            } else {
                this.boss.method_48926().spawnEntity(new EvokerFangsEntity(this.boss.method_48926(), x, (double)blockPos.getY() + d, z, yaw, warmup, this.boss));
            }
        }
    }
    ...
    private void shootSplitProjectile(Vec3d target, int amount, float velocity, EntityType<? extends ProjectileEntity> type) {
        ...
        for (int i = -m; i <= m; i++) {
            ...
            ProjectileEntity entity = type.create(this.boss.method_48926());
            if (entity != null) {
    ...
    private void shootSplitMoonlight(Vec3d target, int amount) {
        ...
        for (int i = -m; i <= m; i++) {
            ...
            MoonlightProjectile entity = new MoonlightProjectile(EntityRegistry.MOONLIGHT_BIG_ENTITY_TYPE, this.boss.method_48926());
    ...
    private void shootSplitBoth(Vec3d target, int amount) {
        ...
        for (int i = -m; i <= m; i++) {
            ...
            if (bl) {
                entity = new NoDragWitherSkull(EntityType.WITHER_SKULL, this.boss.method_48926());
            } else {
                entity = new MoonlightProjectile(EntityRegistry.MOONLIGHT_BIG_ENTITY_TYPE, this.boss.method_48926());
    ...
    private void eclipse() {
        ...
        if (this.attackStatus >= 50 && this.attackStatus <= 220) {
            for (Entity entity : this.boss.method_48926().getOtherEntities(this.boss, this.boss.getBoundingBox().expand(35))) {
                if (this.attackStatus % 12 == 0 && entity instanceof LivingEntity target) {
                    ...
                    if (target.isDead() && target.deathTime < 2) {
                        ...
                        DeathSpiralEntity spiral = new DeathSpiralEntity(this.boss.method_48926(), target.getPos(), 1f);
    ...
    private void engulf(LivingEntity target) {
        ...
        if (this.attackStatus == (phase2 ? 19 : 21)) {
            ...
            FogEntity entity = new FogEntity(EntityRegistry.FOG_ENTITY, this.boss.method_48926());
    ...
    private void blackflameSnake(LivingEntity target) {
        ...
        if (!phase2 && this.attackStatus == 31) {
            this.boss.playSound(SoundRegistry.NIGHT_SKULL_DIE, 1f, 0.75f);
            BlackflameSnakeEntity entity = new BlackflameSnakeEntity(EntityRegistry.BLACKFLAME_SNAKE_ENTITY, this.boss.method_48926());
            entity.setDamage(this.getModifiedDamage(30f));
            Vec3d vel;
            Vec3d pos;
            DayStalker partner;
            if (this.boss.isFlying() && (partner = this.boss.getPartner((ServerWorld) this.boss.method_48926())) != null) {
    ...
    public static class DeathSpiralEntity extends InvisibleEntity {
        ...
        public void tick() {
            ...
            if (this.method_48926().isClient) {
                for (int i = 0; i < 4; i++) {
                    logic.tick(this.method_48926(), this.getPos());
                }
            }
