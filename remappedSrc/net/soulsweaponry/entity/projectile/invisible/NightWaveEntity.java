...
public class NightWaveEntity extends InvisibleEntity {
    ...
    public void tick() {
        ...
        if (this.method_48926().isClient) {
            for (int i = 0; i < 40; ++i) {
                this.method_48926().addParticle(ParticleRegistry.NIGHTFALL_PARTICLE, this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 0.0D, 0.0D, 0.0D);
            }
        } else {
            if (this.age % 4 == 0) {
                ...
                for (LivingEntity living : list) {
                    if (this.getOwner() instanceof LivingEntity) {
                        living.damage(this.method_48926().getDamageSources().mobProjectile(this, (LivingEntity) this.getOwner()), (float) this.getDamage());
                    } else {
                        living.damage(this.method_48926().getDamageSources().mobProjectile(this, null), 20f * ConfigConstructor.night_prowler_damage_modifier);
                    }
                }
            }
            if (this.age % 10 == 0) {
