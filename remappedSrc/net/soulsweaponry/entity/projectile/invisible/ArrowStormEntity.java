...
public class ArrowStormEntity extends InvisibleEntity {
    ...
    public void tick() {
        ...
        if (this.method_48926().isClient) {
            for (int i = 0; i < 60; ++i) {
                this.method_48926().addParticle(ParticleTypes.CLOUD, this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 0.0D, 0.0D, 0.0D);
            }
        } else if (this.getOwner() instanceof LivingEntity) {
            for (int i = 0; i < 6; i++) {
                MoonlightArrow arrow = new MoonlightArrow(this.method_48926(), (LivingEntity) this.getOwner());
                ...
                this.method_48926().spawnEntity(arrow);
            }
        }
        if (this.age > 30) {
            this.discard();
