...
public class MjolnirProjectile extends ReturningProjectile implements GeoEntity {
    ...
    public boolean collide(Entity owner, Entity target, float damage) {
        ...
        int strikes = 1;
        if (this.method_48926().isThundering() || target instanceof LeviathanAxeEntity) strikes = 3;
        if (bl && this.method_48926() instanceof ServerWorld && this.method_48926().isSkyVisible(blockPos = target.getBlockPos())) {
            for (int i = 0; i < strikes; i++) {
                LightningEntity lightningEntity = EntityType.LIGHTNING_BOLT.create(this.method_48926());
