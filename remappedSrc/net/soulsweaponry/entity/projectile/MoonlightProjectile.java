...
public class MoonlightProjectile extends NonArrowProjectile implements GeoEntity {
    ...
    public void tick() {
        ...
        for (int i = 0; i < this.getTickParticleAmount(); ++i) {
            this.method_48926().addParticle(this.getTrailParticleType(), this.getX() + e * (double)i / 4.0D, this.getY() + f * (double)i / 4.0D, this.getZ() + g * (double)i / 4.0D, -e, -f + 0.2D, -g);
        }
    ...
    public void onRemoved() {
        ...
        if (this.dataTracker.get(HUGE_EXPLOSION)) {
            this.detonateEntity(method_48926(), this.getX(), this.getY(), this.getZ(), 750, 0.5f);
        } else {
            this.detonateEntity(method_48926(), this.getX(), this.getY(), this.getZ(), this.getMaxParticlePoints(), 0.125f);
        }
    }
