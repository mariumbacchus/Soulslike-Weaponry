...
public abstract class ReturningProjectile extends PersistentProjectileEntity {
    ...
    public void saveOnPlayer(PlayerEntity player) {
        ...
        try {
            if (player.getDataTracker().get(THROWN_WEAPON_OPT).isPresent() && player.method_48926() instanceof ServerWorld) {
                Entity entity = ((ServerWorld)player.method_48926()).getEntity(player.getDataTracker().get(THROWN_WEAPON_OPT).get());
    ...
    public void tick() {
        ...
        if (this.shouldReturn() && (this.dealtDamage || this.isNoClip()) && entity != null) {
            ...
            this.returnTimer++;
            for (Entity entity1 : this.method_48926().getOtherEntities(this, this.getBoundingBox().expand(0.2D))) {
                if (entity1 instanceof LivingEntity target && !target.isTeammate(entity)) {
                    this.collide(entity, target, this.getDamage(target));
                }
            }
        }
        List<Entity> list = this.method_48926().getOtherEntities(this, this.getBoundingBox()).stream().filter(e -> e instanceof PlayerEntity).toList();
    ...
    private void dropStack() {
        if (!this.method_48926().isClient && this.pickupType == PersistentProjectileEntity.PickupPermission.ALLOWED) {
            this.dropStack(this.asItemStack(), 0.1f);
        }
        this.discard();
    }
