...
public class WitheredWabbajackProjectile extends WitherSkullEntity {
                    ...
                    case 6 -> {
                        ...
                        living.getArmorItems().iterator().forEachRemaining(itemStack -> {
                            ...
                            if (bl) {
                                ...
                                method_48926().playSound(null, living.getBlockPos(), SoundEvents.ENTITY_ARMOR_STAND_BREAK, SoundCategory.PLAYERS, 1f, 1f);
                            }
                        });
                    }
                    default -> {
                        if (power > 50) {
                            method_48926().playSound(null, this.getBlockPos(), SoundRegistry.CRIT_HIT_EVENT, SoundCategory.PLAYERS, .5f, 1f);
                        }
                        target.damage(this.method_48926().getDamageSources().magic(), power);
                    }
                }
            }
        }
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if (type == HitResult.Type.ENTITY) {
            ...
            this.method_48926().emitGameEvent(GameEvent.PROJECTILE_LAND, hitResult.getPos(), GameEvent.Emitter.of(this, null));
        } else if (type == HitResult.Type.BLOCK) {
            BlockHitResult blockHitResult = (BlockHitResult)hitResult;
            this.onBlockHit(blockHitResult);
            BlockPos blockPos = blockHitResult.getBlockPos();
            this.method_48926().emitGameEvent(GameEvent.PROJECTILE_LAND, blockPos, GameEvent.Emitter.of(this, this.method_48926().getBlockState(blockPos)));
        }
    ...
    private void randomCollisionEffect(LivingEntity user) {
        ...
        if (unluckyAf) {
            ...
            for (int i = 0; i < 3; i++) {
                if (isWarden && !this.method_48926().isClient) {
                    WardenEntity warden = new WardenEntity(EntityType.WARDEN, method_48926());
                    warden.initialize((ServerWorldAccess) method_48926(), method_48926().getLocalDifficulty(user.getBlockPos()), SpawnReason.MOB_SUMMONED, null, null);
                    warden.setPos(this.getX(), this.getY(), this.getZ());
                    warden.increaseAngerAt(user, 80, true);
                    method_48926().spawnEntity(warden);
                } else {
                    WitherEntity boss = new WitherEntity(EntityType.WITHER, method_48926());
                    boss.setPos(this.getX(), this.getY(), this.getZ());
                    method_48926().spawnEntity(boss);
                }
            }
            return;
        }
        switch (this.getCollisionEffectType(user)) {
            case BATS -> {
                for (int i = 0; i < power * 2; i++) {
                    BatEntity bat = new BatEntity(EntityType.BAT, method_48926());
                    bat.setPos(this.getX(), this.getY(), this.getZ());
                    this.method_48926().spawnEntity(bat);
                }
            }
            case DARKNESS -> {
                method_48926().playSound(null, this.getBlockPos(), SoundEvents.AMBIENT_CAVE.value(), SoundCategory.AMBIENT, 1f, 1f);
                method_48926().playSound(null, this.getBlockPos(), SoundEvents.ENTITY_WARDEN_EMERGE, SoundCategory.HOSTILE, 1f, 1f);
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.DARKNESS, 60, 0));
            }
            case LIGHTNING -> {
                for (int i = 0; i < power; i++) {
                    LightningEntity entity = new LightningEntity(EntityType.LIGHTNING_BOLT, method_48926());
            ...
            case PARTICLES -> {
                if (!this.method_48926().isClient) {
                    ...
                    ParticleHandler.particleSphereList(this.method_48926(), amount, this.getX(), this.getY(), this.getZ(), 1f, particle);
                }
            }
            case RANDOM_ENTITY -> this.summonRandomEntity(user, power);
            case SPECIFIC_ENTITY -> this.summonSpecificEntity(user, power);
            case CURSE -> {
                this.method_48926().addParticle(ParticleTypes.ELDER_GUARDIAN, this.getX(), this.getY(), this.getZ(), 0f, 0f, 0f);
                this.method_48926().playSound(null, user.getBlockPos(), SoundEvents.ENTITY_ELDER_GUARDIAN_CURSE, SoundCategory.HOSTILE, 1f, 1f);
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.WEAKNESS, 200, 5));
            }
            default -> {
                boolean bl = method_48926().getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING);
                this.method_48926().createExplosion(this.getOwner(), this.getX(), this.getY(), this.getZ(), power, bl, bl ? World.ExplosionSourceType.TNT : World.ExplosionSourceType.NONE);
            }
        }
    }
    ...
    private void summonSpecificEntity(LivingEntity user, int power) {
        if (!this.method_48926().isClient) {
            ...
            for (int i = 0; i < power; i++) {
                type.spawn((ServerWorld) this.method_48926(), this.getBlockPos(), SpawnReason.EVENT);
            }
        }
    }
