...
public class GrowingFireball extends UntargetableFireball implements GeoEntity {
    ...
    public void tick() {
        ...
        if (method_48926().isClient && this.random.nextBoolean()) {
            int points = MathHelper.floor(this.getRadius() * 4f);
            AreaEffectSphere.randomParticleBox(this.method_48926(), this.getX(), this.getY() + this.getRadius()/2, this.getZ(), points, this.getRadius() * 0.5f, ParticleTypes.FLAME, this.random);
        }
        ...
        Entity target;
        if (!this.method_48926().isClient && this.age >= this.getMaxAge() && !this.hasChangedCourse && (target = this.getSavedTarget((ServerWorld) this.method_48926())) != null) {
        ...
        if (!(this.getOwner() instanceof DayStalker) && !this.method_48926().isClient) {
            for (Entity entity : this.method_48926().getOtherEntities(this, this.getBoundingBox())) {
    ...
    private void detonate() {
        if (!this.method_48926().isClient) {
            boolean bl = this.method_48926().getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING);
            this.method_48926().createExplosion(this, this.getX(), this.getY(), this.getZ(), this.getExplosionPower(), bl, World.ExplosionSourceType.MOB);
            this.discard();
        }
    }
