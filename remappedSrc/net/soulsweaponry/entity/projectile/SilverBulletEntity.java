...
public class SilverBulletEntity extends NonArrowProjectile implements GeoEntity {
    ...
    public void tick() {
        ...
        if (!this.inGround) {
            ...
            double g = vec3d.z;
            for (int i = 0; i < 2; ++i) {
                this.method_48926().addParticle(ParticleTypes.SMOKE, this.getX() + e * (double)i / 4.0D, this.getY() + f * (double)i / 4.0D, this.getZ() + g * (double)i / 4.0D, -e*0.2, (-f + 0.2D)*0.2, -g*0.2);
            }
        }
        Vec3d vec3d = this.getVelocity();
