...
public class ChaosSkull extends WitherSkullEntity {
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        if (entityHitResult.getEntity() != null && entityHitResult.getEntity() instanceof LivingEntity entity && this.getOwner() instanceof LivingEntity) {
            ...
            if (damage > this.getModifiedDamage(20)) {
                this.method_48926().playSound(null, this.getBlockPos(), SoundRegistry.CRIT_HIT_EVENT, SoundCategory.HOSTILE, .5f, 1f);
            }
            entity.damage(this.method_48926().getDamageSources().mobAttack((LivingEntity)this.getOwner()), damage);
        }
    }
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if (this.getOwner() instanceof LivingEntity && !this.method_48926().isClient) {
            int amount = this.random.nextInt(5) + 1;
            int rng = this.random.nextInt(6);
            for (int i = 0; i < amount; i++) {
                switch (rng) {
                    case 1 -> {
                        WitherSkeletonEntity skeleton = new WitherSkeletonEntity(EntityType.WITHER_SKELETON, this.method_48926());
                        skeleton.setPos(this.getX(), this.getY(), this.getZ());
                        this.initEquip(skeleton);
                        this.method_48926().spawnEntity(skeleton);
                    }
                    case 2 -> {
                        LightningEntity lightningEntity = new LightningEntity(EntityType.LIGHTNING_BOLT, method_48926());
                        lightningEntity.setPos(this.getX(), this.getY(), this.getZ());
                        this.method_48926().spawnEntity(lightningEntity);
                    }
                    case 3 -> this.randomHostile().spawn((ServerWorld) this.method_48926(), this.getBlockPos(), SpawnReason.EVENT);
                    case 4 -> this.randomPassive().spawn((ServerWorld) this.method_48926(), this.getBlockPos(), SpawnReason.EVENT);
                    case 5 -> {
                        float power = this.random.nextFloat() * 3;
                        this.method_48926().createExplosion(this, this.method_48926().getDamageSources().mobAttack((LivingEntity) this.getOwner()), new ExplosionBehavior(), this.getX(), this.getY(), this.getZ(), power, false, World.ExplosionSourceType.TNT);
                    }
                    default -> {
                        for (int j = 0; j < 2; j++) {
                            BigChungus chungus = new BigChungus(EntityRegistry.BIG_CHUNGUS, this.method_48926());
                            chungus.setPos(this.getX(), this.getY(), this.getZ());
                            this.method_48926().spawnEntity(chungus);
                        }
                    }
                }
            }
    ...
    private void finisher() {
        this.method_48926().playSound(null, this.getBlockPos(), SoundEvents.ENTITY_WITHER_BREAK_BLOCK, SoundCategory.HOSTILE, 1f, 1f);
        if (!this.method_48926().isClient) {
            ParticleHandler.particleSphereList(this.method_48926(), 100, this.getX(), this.getY(), this.getZ(), ParticleEvents.DARK_EXPLOSION_LIST, 1f);
        }
        this.discard();
    }
