...
public class LeviathanAxeEntity extends ReturningProjectile implements GeoEntity {
    ...
    public boolean collide(Entity owner, Entity target, float damage) {
        if (!this.method_48926().isClient && target instanceof MjolnirProjectile) {
            ParticleEvents.mjolnirLeviathanAxeCollision(this.method_48926(), this.getX(), this.getY(), this.getZ());
            this.method_48926().createExplosion(null, this.getX(), this.getY(), this.getZ(), 6.0F, true, World.ExplosionSourceType.TNT);
        }
        DamageSource damageSource = this.method_48926().getDamageSources().trident(this, owner);
        boolean bl = target.damage(damageSource, damage);
        if (bl) {
            LeviathanAxe.iceExplosion(method_48926(), this.getBlockPos(), this.getOwner(), EnchantmentHelper.getLevel(Enchantments.SHARPNESS, this.stack));
        }
        return bl;
    }
