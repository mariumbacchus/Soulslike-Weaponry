...
public class DayStalker extends BossEntity implements GeoEntity {
    ...
    public void updatePostDeath() {
        ...
        if (this.deathTicks == this.getTicksUntilDeath() && !this.method_48926().isClient()) {
            ...
            CustomDeathHandler.deathExplosionEvent(this.method_48926(), this.getPos(), SoundRegistry.DAWNBREAKER_EVENT, ParticleTypes.FLAME, ParticleTypes.LARGE_SMOKE);
    ...
    public boolean isEmpowered() {
        return (!this.method_48926().isClient && this.method_48926().isDay()) || this.isPhaseTwo();
    }
    ...
    protected void mobTick() {
        ...
        if (this.isInitiatingPhaseTwo()) {
            ...
            if (this.phaseTwoTicks == 76) {
                this.method_48926().playSound(null, this.getBlockPos(), SoundRegistry.DAY_STALKER_RADIANCE, SoundCategory.HOSTILE, 1f, 1f);
            }
            if (this.phaseTwoTicks == 81) {
                if (!method_48926().isClient) {
                    ParticleHandler.particleSphereList(this.method_48926(), 1000, this.getX(), this.getY(), this.getZ(), 1f, ParticleTypes.FLAME, ParticleTypes.LARGE_SMOKE);
                }
        ...
        if (this.getAttackAnimation().equals(Attacks.SPAWN)) {
            ...
            if (this.spawnTicks >= this.maxSpawnTicks) {
                ...
                if (!this.method_48926().isClient && (partner = this.getPartner((ServerWorld) this.method_48926())) != null) {
    ...
    public void tickMovement() {
        super.tickMovement();
        if (this.getParticleState() == 1 && this.method_48926().isClient) {
            ...
            for (int i = 0; i < 50; i++) {
                ...
                this.method_48926().addParticle(ParticleTypes.FLAME, this.getX(), this.getEyeY(), this.getZ(), velX / 10, velY / 10, velZ / 10);
            }
        }
        if (this.getAttackAnimation().equals(Attacks.FLAMES_EDGE)) {
            ...
            for (int theta = 0; theta < 360; theta += this.isPhaseTwo() ? 4 : 8) {
                ...
                if (this.getParticleState() == 2 && this.age % 8 == 0) {
                    this.method_48926().addParticle(ParticleTypes.FLAME, x, this.getBodyY(0.5D), z, 0, 0, 0);
                } else if (this.getParticleState() == 3) {
                    this.method_48926().addParticle(ParticleTypes.SOUL_FIRE_FLAME, x, this.getBodyY(0.5D), z,
                            this.random.nextGaussian()/8, this.random.nextGaussian()/8, this.random.nextGaussian()/8);
                    this.method_48926().addParticle(ParticleTypes.LARGE_SMOKE, x, this.getBodyY(0.5D), z,
                            0, 0.2f, 0);
                }
            }
        }
