...
public class AccursedLordBoss extends BossEntity implements GeoEntity {
    ...
    public void updatePostDeath() {
        ...
        if (this.deathTicks >= this.getTicksUntilDeath() && !this.method_48926().isClient) {
            ...
            CustomDeathHandler.deathExplosionEvent(this.method_48926(), this.getPos(), SoundRegistry.DAWNBREAKER_EVENT, ParticleTypes.LARGE_SMOKE, ParticleTypes.FLAME);
    ...
    public void tickMovement() {
        ...
        if (this.getAttackAnimation().equals(AccursedLordAnimations.SPAWN)) {
            ...
            for(int i = 0; i < 50; ++i) {
                ...
                method_48926().addParticle(ParticleTypes.FLAME, pos.getX(), pos.getY(), pos.getZ(), newX/2, newY/6, newZ/2);
                method_48926().addParticle(ParticleTypes.LARGE_SMOKE, pos.getX(), pos.getY(), pos.getZ(), newX/2, newY/6, newZ/2);
            }
            ...
            if (this.spawnTicks > 110 && this.spawnTicks <= 112) {
                ...
                for (Entity nearbyEntity : nearbyEntities) {
                    if (nearbyEntity instanceof LivingEntity closestTarget) {
                        ...
                        closestTarget.damage(this.method_48926().getDamageSources().mobAttack(this), 50f * ConfigConstructor.decaying_king_damage_modifier);
                    }
                }
                if (!this.method_48926().isClient) {
                    ParticleHandler.particleSphere(this.method_48926(), 1000, this.getX(), this.getY() + 1f, this.getZ(), ParticleTypes.FLAME, 1f);
                    ParticleHandler.particleOutburstMap(this.method_48926(), 200, this.getX(), this.getY() + .1f, this.getZ(), ParticleEvents.DAWNBREAKER_MAP, 1f);
                }
            }
    ...
    public void removePlacedLava() {
        for (BlockPos pos : this.lavaPos) {
            if (this.method_48926().getBlockState(pos).isOf(Blocks.LAVA)) {
                this.method_48926().setBlockState(pos, Blocks.AIR.getDefaultState());
            }
        }
        this.lavaPos.clear();
    }
