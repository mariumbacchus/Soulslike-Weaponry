...
public class NightShade extends BossEntity implements GeoEntity {
    ...
    public void tickMovement() {
        ...
        if (!this.isCopy && !this.hasDuplicated && this.getHealth() <= this.getMaxHealth() / 2.0F) {
            ...
            if (this.duplicateTicks == 20) {
                CustomDeathHandler.deathExplosionEvent(this.method_48926(), this.getPos(), SoundRegistry.NIGHTFALL_SPAWN_EVENT, ParticleTypes.LARGE_SMOKE, ParticleRegistry.NIGHTFALL_PARTICLE, ParticleRegistry.DARK_STAR);
                this.getNavigation().stop();
                for (int i = -1; i <= 1; i += 2) {
                    NightShade copy = new NightShade(EntityRegistry.NIGHT_SHADE, this.method_48926());
                    copy.setCopy(true);
                    ...
                    method_48926().spawnEntity(copy);

                    NightShade copy2 = new NightShade(EntityRegistry.NIGHT_SHADE, this.method_48926());
                    copy2.setCopy(true);
                    ...
                    method_48926().spawnEntity(copy2);
                }
            }
    ...
    public void updatePostDeath() {
        ...
        if (this.deathTicks % 30 == 0) {
            this.method_48926().playSound(null, this.getBlockPos(), SoundEvents.ENTITY_WITHER_AMBIENT, SoundCategory.HOSTILE, 1f, 1f);
        }
        if (this.deathTicks >= this.getTicksUntilDeath() && !this.method_48926().isClient()) {
            ...
            this.method_48926().sendEntityStatus(this, EntityStatuses.ADD_DEATH_PARTICLES);
            CustomDeathHandler.deathExplosionEvent(this.method_48926(), this.getPos(), SoundRegistry.DAWNBREAKER_EVENT, ParticleTypes.LARGE_SMOKE, ParticleRegistry.NIGHTFALL_PARTICLE, ParticleRegistry.DARK_STAR);
            this.remove(RemovalReason.KILLED);
        }
    }
    ...
    class Emerge extends Goal {
        ...
        private boolean isInsideWall() {
            ...
            return BlockPos.stream(box).anyMatch(pos -> {
                BlockState blockState = NightShade.this.method_48926().getBlockState(pos);
                return !blockState.isAir() && blockState.shouldSuffocate(NightShade.this.method_48926(), pos) && VoxelShapes.matchesAnywhere(blockState.getCollisionShape(NightShade.this.method_48926(), pos).offset(pos.getX(), pos.getY(), pos.getZ()), VoxelShapes.cuboid(box), BooleanBiFunction.AND);
            });
        }
    }
