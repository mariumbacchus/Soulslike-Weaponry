...
public class DraugrBoss extends BossEntity implements GeoEntity {
    ...
    public void tickMovement() {
        ...
        if (this.isSpawning()) {
            ...
            for(int i = 0; i < 30; ++i) {
                ...
                method_48926().addParticle(ParticleTypes.SOUL, pos.getX(), pos.getY(), pos.getZ(), newX/2, newY/8, newZ/2);
                method_48926().addParticle(ParticleTypes.LARGE_SMOKE, pos.getX(), pos.getY(), pos.getZ(), newX/2, newY/8, newZ/2);
            }
            ...
            if (this.spawnTicks == 48 || this.spawnTicks == 55) {
                this.method_48926().playSound(null, this.getBlockPos(), SoundRegistry.SWORD_HIT_SHIELD_EVENT, SoundCategory.HOSTILE, 1f, 1f);
                for (Entity entity : this.method_48926().getOtherEntities(this, getBoundingBox().expand(10f))) {
        ...
        if (this.getHealth() <= this.getMaxHealth() / 2.0F) {
        ...
        } else if (this.method_48926().isSkyVisible(this.getBlockPos())) {
            ...
            for(int i = 0; i < 2; ++i) {
                this.method_48926().addParticle(ParticleTypes.SNOWFLAKE, this.getParticleX(0.5D), this.getRandomBodyY(), this.getParticleZ(0.5D), 0.0D, 0.0D, 0.0D);
            }
        }
    ...
    */
    ...
    public void onDeath(DamageSource source) {
        ...
        CustomDeathHandler.deathExplosionEvent(this.method_48926(), this.getPos(), SoundRegistry.NIGHTFALL_SPAWN_EVENT, ParticleTypes.LARGE_SMOKE, ParticleTypes.SOUL_FIRE_FLAME, ParticleRegistry.BLACK_FLAME);
        NightShade entity = new NightShade(EntityRegistry.NIGHT_SHADE, method_48926());
        entity.setPos(this.getX(), this.getY() + .1F, this.getZ());
        entity.setVelocity(0, .1f, 0);
        entity.setSpawn();
        method_48926().spawnEntity(entity);
    }
