...
public class NightProwler extends BossEntity implements GeoEntity {
    ...
    public void updatePostDeath() {
        ...
        if (this.deathTicks == this.getTicksUntilDeath() && !this.method_48926().isClient()) {
            ...
            CustomDeathHandler.deathExplosionEvent(this.method_48926(), this.getPos(), SoundRegistry.DAWNBREAKER_EVENT, ParticleTypes.LARGE_SMOKE, ParticleTypes.SOUL_FIRE_FLAME);
    ...
    public boolean isEmpowered() {
        return (!this.method_48926().isClient && this.method_48926().isNight()) || this.isPhaseTwo();
    }
    ...
    protected void mobTick() {
        ...
        if (!this.method_48926().isClient) {
            LivingEntity partner = this.getPartner((ServerWorld) this.method_48926());
        ...
        if (this.isInitiatingPhaseTwo()) {
            ...
            if (this.phaseTwoTicks == 78) {
                this.method_48926().playSound(null, this.getBlockPos(), SoundRegistry.PARTNER_DIES, SoundCategory.HOSTILE, 1f, 1f);
            }
            if (this.phaseTwoTicks == 81) {
                this.method_48926().playSound(null, this.getBlockPos(), SoundRegistry.DAWNBREAKER_EVENT, SoundCategory.HOSTILE, 1f, 1f);
                if (!method_48926().isClient) {
                    ParticleHandler.particleSphereList(this.method_48926(), 1000, this.getX(), this.getY(), this.getZ(), 1f, ParticleTypes.SOUL_FIRE_FLAME, ParticleTypes.LARGE_SMOKE);
                }
        ...
        if (this.getDarknessRise()) {
            ...
            for (Entity entity : this.method_48926().getOtherEntities(this, box)) {
                if (this.darknessRiseTicks % 4 == 0 && entity instanceof LivingEntity living) {
                    ...
                    living.damage(this.method_48926().getDamageSources().magic(), 1f);
                }
            }
        ...
        if (this.getBlackflameSnakeLogic() != null) {
            this.getBlackflameSnakeLogic().tick(this.method_48926());
    ...
    public boolean teleportTo(double x, double y, double z) {
        ...
        while (mutable.getY() > this.method_48926().getBottomY() && !this.method_48926().getBlockState(mutable).blocksMovement()) {
            mutable.move(Direction.DOWN);
        }
        BlockState blockState = this.method_48926().getBlockState(mutable);
        ...
        if (bl3) {
            this.method_48926().emitGameEvent(GameEvent.TELEPORT, vec3d, GameEvent.Emitter.of(this));
            if (!this.isSilent()) {
                this.method_48926().playSound(null, this.prevX, this.prevY, this.prevZ, SoundEvents.ENTITY_ENDERMAN_TELEPORT, SoundCategory.HOSTILE, 1.0f, 1.0f);
    ...
    public boolean teleportAway() {
        if (this.method_48926().isClient() || !this.isAlive()) {
            return false;
        }
    ...
    public void tickMovement() {
        ...
        if (this.method_48926().isClient) {
            if (this.getHealth() < this.getMaxHealth() * ConfigConstructor.night_prowler_projectile_heal_below_percent_health) {
                for (int i = 0; i < 2; i++) {
                    this.method_48926().addParticle(ParticleRegistry.DAZZLING_PARTICLE, this.getParticleX(0.5D), this.getRandomBodyY(), this.getParticleZ(0.5D), 0.0D, 0.0D, 0.0D);
                }
            }
                ...
                case 1 -> {
                    if (this.getTargetPos() != null) {
                        for (int i = 0; i < 500; ++i) {
                            ...
                            this.method_48926().addParticle(ParticleTypes.SOUL_FIRE_FLAME, pos.getX(), pos.getY(), pos.getZ(), newX, newY, newZ);
                            this.method_48926().addParticle(ParticleRegistry.NIGHTFALL_PARTICLE, pos.getX(), pos.getY(), pos.getZ(), newX, newY, newZ);
                            this.method_48926().addParticle(ParticleTypes.LARGE_SMOKE, pos.getX(), pos.getY(), pos.getZ(), newX, newY, newZ);
                        }
                    }
                }
                case 2 -> {
                    if (this.getTargetPos() != null) {
                        for (int i = 0; i < 250; ++i) {
                            ...
                            this.method_48926().addParticle(ParticleTypes.LARGE_SMOKE, pos.getX(), pos.getY(), pos.getZ(), newX, newY, newZ);
                            this.method_48926().addParticle(ParticleTypes.CLOUD, pos.getX(), pos.getY(), pos.getZ(), newX, newY, newZ);
                            this.method_48926().addParticle(ParticleRegistry.DARK_STAR, pos.getX(), pos.getY(), pos.getZ(), newX, newY, newZ);
                        }
                    }
                }
                case 3 -> {
                    for (int t = -90; t < 90; t++) {
                        ...
                        int div = 75;
                        this.method_48926().addParticle(ParticleRegistry.DARK_STAR, vec1.x, vec1.y, vec1.z,
                                this.random.nextGaussian()/div, this.random.nextGaussian()/div, this.random.nextGaussian()/div);
                        this.method_48926().addParticle(ParticleRegistry.DAZZLING_PARTICLE, vec2.x, vec2.y, vec2.z,
                                this.random.nextGaussian()/div, this.random.nextGaussian()/div, this.random.nextGaussian()/div);
                    }
                }
                case 4 -> {
                    ...
                    for (int i = 0; i < points; i++) {
                        ...
                        this.method_48926().addParticle(ParticleRegistry.DARK_STAR, true, x * sizeMod + this.getX(), y * sizeMod + this.getY(), z * sizeMod + this.getZ(),
                                this.random.nextGaussian()/div, this.random.nextGaussian()/div, this.random.nextGaussian()/div);
                    }
                }
                default -> {}
                ...
                for (int i = 0; i < 12; i++) {
                    ...
                    for (int theta = 0; theta < 360; theta++) {
                        if (theta % 8 == 0) {
                            ...
                            this.method_48926().addParticle(ParticleRegistry.DARK_STAR, x, this.getY() + 0.1f, z,
                                    this.random.nextGaussian()/div, this.random.nextGaussian()/div, this.random.nextGaussian()/div);
                        }
                    }
                }
