...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠉⠒⠤⢤⣀⣀⡤⠴⠒⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
    ...
    public boolean canSpawn(WorldView view) {
        ...
        return view.doesNotIntersectEntities(this) && this.method_48926().isNight() && !method_48926().containsFluid(this.getBoundingBox())
            && state.getBlock().canMobSpawnInside(state)
            && this.method_48926().getBlockState(blockUnderEntity).allowsSpawning(view, blockUnderEntity, EntityRegistry.BIG_CHUNGUS)
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                if (x * x + z * z <= radius * radius) {
                    ...
                    Block block = this.method_48926().getBlockState(checkPos).getBlock();
