...
public class WitheredDemon extends HostileEntity implements GeoEntity, IAnimatedDeath {
    ...
    public boolean canSpawn(WorldView view) {
        ...
        BlockState state = this.method_48926().getBlockState(positionEntity);
        return view.doesNotIntersectEntities(this) && !method_48926().containsFluid(this.getBoundingBox())
            ...
            && !method_48926().getBlockState(positionEntity.down()).isOf(Blocks.NETHER_WART_BLOCK)
            && method_48926().getDifficulty() != Difficulty.PEACEFUL
            && method_48926().getBlockState(positionEntity.down()).isOf(Blocks.CRIMSON_NYLIUM)
            && this.method_48926().getBlockState(blockUnderEntity).allowsSpawning(view, blockUnderEntity, EntityRegistry.WITHERED_DEMON)
            && this.isSpawnable();
    }
    ...
    public boolean tryAttack(Entity target) {
        ...
        boolean bl = target.damage(this.method_48926().getDamageSources().mobAttack(this), g);
        if (bl) {
    ...
    public void tickMovement() {
        ...
        if (ran < 0.05D) {
            this.method_48926().addParticle(ParticleTypes.FLAME, this.getX(), this.getY() + 1.4F, this.getZ(), ran - 0.025D, ran - 0.025D, ran - 0.025D);
        }
    }
