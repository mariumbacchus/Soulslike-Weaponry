...
public class DarkSorcerer extends HostileEntity {
    ...
    public boolean canSpawn(WorldView view) {
        ...
        BlockState state = this.method_48926().getBlockState(this.getBlockPos());
        return view.doesNotIntersectEntities(this) && !method_48926().containsFluid(this.getBoundingBox())
                && state.getBlock().canMobSpawnInside(state)
                && this.method_48926().getBlockState(blockUnderEntity).isOf(Blocks.DEEPSLATE_TILES);
    }
            ...
            if (target != null) {
                ...
                if (target instanceof ReturningKnight) {
                    ...
                    if (distanceToEntity < 150f) {
                        ...
                        this.user.setBeamCords(target.getBlockX(), target.getEyeY(), target.getBlockZ());
                        if (attackTicks < 0) {
                            target.damage(this.user.method_48926().getDamageSources().mobAttack(user), 2f);
                            attackTicks = 10;
                        }
                    } else {
                        this.user.setBeaming(false);
    ...
    public void tickMovement() {        
        if (this.getBeaming()) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                this.method_48926().addParticle(ParticleTypes.ELECTRIC_SPARK, this.getX() + e * length, this.getEyeY() + f * length, this.getZ() + g * length, 0.0D, 0.0D, 0.0D);
                this.method_48926().addParticle(ParticleTypes.GLOW, this.getX() + e * length, this.getEyeY() + f * length, this.getZ() + g * length, 0.0D, 0.0D, 0.0D);
            }
            ...
            this.method_48926().addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() + (double)cosBody * 0.5D, this.getY() + 1.8D, this.getZ() + (double)sinBody * 0.5D, newX, newY, newZ);
            this.method_48926().addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() - (double)cosBody * 0.5D, this.getY() + 1.8D, this.getZ() - (double)sinBody * 0.5D, newX, newY, newZ);
        }
        super.tickMovement();
    }
