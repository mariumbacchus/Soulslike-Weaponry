...
import net.soulsweaponry.networking.packets.S2C.FrostSyncS2C;
...
public class FrostData {
    ...
    public static void addFrost(LivingEntity entity, int amount) {
        if (!EntityFrost.isFrostBuildupDisabled(entity) && !entity.isOnFire() && !isFrostCoolingDown(entity) && !entity.isDead() && !entity.getWorld().isClient) {
            ...
            addFrost(entity, newAmount, EntityFrost.getMaxFrostBuildup(entity));
        }
    }

    private static int addFrost(LivingEntity entity, int amount, int max) {
     ...
     */
    public static int reduceFrost(LivingEntity entity, int amount, int max) {
        return addFrost(entity, -amount, max);
    }

    public static int setFrost(LivingEntity entity, int amount, boolean frostCoolingDown) {
        NbtCompound nbt = entity.getPersistentData();
    ...
    public static boolean setFrostCoolingDown(LivingEntity entity, boolean coolingDown) {
        NbtCompound nbt = entity.getPersistentData();
    ...
    public static void syncFrostData(int value, boolean frostCoolingDown, ServerPlayerEntity entity) {
        ModMessages.sendToPlayer(new FrostSyncS2C(value, frostCoolingDown), entity);
    }
     ...
     * targeted or not.
     */
    public static UUID getFrostSource(Entity target) {
