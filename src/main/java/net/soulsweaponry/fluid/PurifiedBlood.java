...
import net.soulsweaponry.registry.FluidTypeRegistry;
...
public abstract class PurifiedBlood extends FlowableFluid {
    ...
    public Fluid getStill() {
        return FluidRegistry.STILL_PURIFIED_BLOOD.get();
    }

    @Override
    public Fluid getFlowing() {
        return FluidRegistry.FLOWING_PURIFIED_BLOOD.get();
    }

    @Override
    public Item getBucketItem() {
        return ItemRegistry.PURIFIED_BLOOD_BUCKET.get();
    }

    @Override
    public FluidType getFluidType() {
        return FluidTypeRegistry.PURIFIED_BLOOD_FLUID_TYPE.get();
    }

    @Override
    protected BlockState toBlockState(FluidState state) {
        return BlockRegistry.PURIFIED_BLOOD_BLOCK.get().getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
