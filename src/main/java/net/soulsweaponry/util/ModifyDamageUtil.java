...
public class ModifyDamageUtil {
    ...
    public static float modifyDamageTakenTail(LivingEntity entity, float newAmount, DamageSource source) {
        if (entity.hasStatusEffect(EffectRegistry.DECAY.get())) {
            boolean immune = true;
        ...
        if (entity.hasStatusEffect(EffectRegistry.POSTURE_BREAK.get()) && !source.isIn(DamageTypeTags.IS_PROJECTILE)) {
            ...
            if (entity.hasStatusEffect(EffectRegistry.FREEZING.get())) {
                int amp = entity.getStatusEffect(EffectRegistry.FREEZING.get()).getAmplifier();
    ...
    private static boolean isDecayImmune(ItemStack stack) {
        ...
        if (stack.getItem() instanceof IHasAbilities hasAbilities) {
            immune = hasAbilities.getAbilities().stream().anyMatch(a -> a.getStatusEffectsImmuneTo().contains(EffectRegistry.DECAY.get()));
        }
        return immune;
    }
}