...
public class BigChungus extends TameableEntity implements InventoryOwner {
    ...
    protected void initGoals() {
        ...
        this.targetSelector.add(3, new ActiveTargetGoal<>(this, PlayerEntity.class, true, p -> (!p.hasStatusEffect(EffectRegistry.CHUNGUS_TONIC_EFFECT.get()) || this.isAggressive()) && !this.isTamed()));
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (ConfigConstructor.can_chungus_barter && stack.isOf(ItemRegistry.CHUNGUS_EMERALD.get()) && this.getInventory().isEmpty() && !this.isAggressive()) {
            this.addItem(stack);
        ...
        if (stack.isOf(WeaponRegistry.CHUNGUS_STAFF.get()) && !((IConfigDisable)stack.getItem()).isDisabled(stack) && !this.isAggressive() && !this.isTamed()) {
            this.setTamed(true);
    ...
    protected void loot(ItemEntity item) {
        if (item.getStack().isOf(ItemRegistry.CHUNGUS_EMERALD.get())) {
