...
public class DayStalker extends BossEntity<DayStalker.States> implements GeoEntity {
    ...
    public DayStalker(EntityType<? extends DayStalker> entityType, World world) {
        super(entityType, world, Color.YELLOW, States.class);
    }
    ...
    public boolean isFireImmune() {
        return EntityConfig.day_stalker_is_fire_immune;
    }

    @Override
    public boolean hasInvertedHealingAndHarm() {
        return EntityConfig.day_stalker_has_inverted_heal_and_harm;
    }

    @Override
    public String[] getBlacklistedStatusEffects() {
        return EntityConfig.day_stalker_status_effect_blacklist;
    }

    @Override
    public boolean disablesShield() {
        return EntityConfig.day_stalker_disables_shields;
    }
    ...
    public static DefaultAttributeContainer.Builder createBossAttributes() {
        return HostileEntity.createHostileAttributes()
                ...
                .add(EntityAttributes.GENERIC_MAX_HEALTH, EntityConfig.day_stalker_health)
                .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.3D)
                .add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 20.0D)
                .add(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, 10.0D)
                .add(EntityAttributes.GENERIC_ARMOR, EntityConfig.day_stalker_armor)
    ...
    @Override
    public int getXp() {
        return (int) EntityConfig.day_stalker_xp;
    }
    ...
    public void tickMovement() {
        ...
        if (this.getState().equals(States.FLAMES_EDGE)) {
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        if (this.getState().equals(States.OVERHEAT)) {
            amount = amount * 0.6f;
        }
        if (this.isEmpowered() && source.isIn(DamageTypeTags.IS_PROJECTILE) && !this.isFlying()) {
            amount = amount * (this.isPhaseTwo() ? EntityConfig.day_stalker_empowered_projectile_damage_taken_modifier_phase_2 :
                    EntityConfig.day_stalker_empowered_projectile_damage_taken_modifier_phase_1);
            if (source.getAttacker() instanceof LivingEntity attacker && this.getHealth() < this.getMaxHealth() * EntityConfig.day_stalker_projectile_cause_air_combustion_below_percent_health) {
                AirCombustion airCombustion = new AirCombustion(this.getWorld(), this, 35f * EntityConfig.day_stalker_damage_modifier, this.isPhaseTwo() ? 5f : 3f, 12);
