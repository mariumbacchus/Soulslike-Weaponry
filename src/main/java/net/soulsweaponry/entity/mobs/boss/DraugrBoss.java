...
public class DraugrBoss extends BossEntity<DraugrBoss.States> implements GeoEntity {
    ...
    public static DefaultAttributeContainer.Builder createBossAttributes() {
        ...
        .add(EntityAttributes.GENERIC_MAX_HEALTH, EntityConfig.old_champions_remains_health)
        .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.23D)
        .add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 10D)
        .add(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, 1.0D)
        .add(EntityAttributes.GENERIC_ARMOR, EntityConfig.old_champions_remains_armor);
    }
    ...
    */
    ...
    public boolean damage(DamageSource source, float amount) {
        if (source.getAttacker() instanceof LivingEntity attacker) {
            ...
            if (this.getSameWeaponCount() >= EntityConfig.old_champions_remains_hits_before_growing_resistant) {
                double x = this.getSameWeaponCount() - (EntityConfig.old_champions_remains_hits_before_growing_resistant -
        ...
        if (source.getSource() instanceof ProjectileEntity projectile) {
            if (this.projectileCount <= 0) {
                if (!this.getWorld().isClient) {
                    ParticleHandler.particleSphereList(this.getWorld(), 30, this.getX(), this.getBodyY(0.5f), this.getZ(), 0.3f, ParticleRegistry.NIGHTFALL_PARTICLE, ParticleRegistry.BLUE_FLAME);
                }
            ...
            this.projectileCount = (int) EntityConfig.old_champions_remains_max_projectile_hits_before_immune;
        }
    ...
    @Override
    public int getXp() {
        return (int) EntityConfig.old_champions_remains_xp;
    }
    ...
    public boolean hasInvertedHealingAndHarm() {
        return EntityConfig.old_champions_remains_has_inverted_heal_and_harm;
    }

    @Override
    public String[] getBlacklistedStatusEffects() {
        return EntityConfig.old_champions_remains_status_effect_blacklist;
    }

    @Override
    public boolean isFireImmune() {
        return EntityConfig.old_champions_remains_is_fire_immune;
    }
    ...
    public boolean disablesShield() {
        return EntityConfig.old_champions_remains_disables_shields && this.shouldDisableShield || EntityConfig.old_champions_remains_disables_shields_all_attacks;
    }
