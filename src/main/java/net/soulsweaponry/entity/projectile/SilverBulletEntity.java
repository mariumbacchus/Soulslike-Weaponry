...
public class SilverBulletEntity extends NonArrowProjectile implements GeoEntity, IPostureLossProjectile {
    ...
    public void tick() {
        ...
        if (!this.inGround) {
            ...
            double g = vec3d.z;
            for (int i = 0; i < 2; ++i) {
                this.getWorld().addParticle(this.isEchoCopy() ? ParticleRegistry.ECHO_SMOKE.get() : ParticleTypes.SMOKE, this.getX() + e * (double)i / 4.0D, this.getY() + f * (double)i / 4.0D + 0.25f, this.getZ() + g * (double)i / 4.0D, -e*0.2, (-f + 0.2D)*0.2, -g*0.2);
            }
        }
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        if (entityHitResult.getEntity() instanceof LivingEntity target) {
            ...
            if (this.getBlightCarrier() > 0) {
                StatusEffectInstance instance = target.getStatusEffect(EffectRegistry.BLIGHT.get());
                if (instance != null) {
                    target.addStatusEffect(new StatusEffectInstance(EffectRegistry.BLIGHT.get(), (int) ConfigConstructor.blight_carrier_enchant_blight_duration, instance.getAmplifier() + this.getBlightCarrier() - 1));
                } else {
                    target.addStatusEffect(new StatusEffectInstance(EffectRegistry.BLIGHT.get(), (int) ConfigConstructor.blight_carrier_enchant_blight_duration, this.getBlightCarrier() - 1));
                }
            }
        }
