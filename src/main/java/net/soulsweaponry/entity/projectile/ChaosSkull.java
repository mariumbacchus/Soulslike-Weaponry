    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        if (entityHitResult.getEntity() != null && entityHitResult.getEntity() instanceof LivingEntity entity && this.getOwner() instanceof LivingEntity) {
            ...
            if (damage > this.getModifiedDamage(20)) {
                this.getWorld().playSound(null, this.getBlockPos(), SoundRegistry.CRIT_HIT_EVENT.get(), SoundCategory.HOSTILE, .5f, 1f);
            }
        ...
        if (this.getOwner() instanceof LivingEntity && !this.getWorld().isClient) {
            ...
            for (int i = 0; i < amount; i++) {
                switch (rng) {
                    ...
                    default -> {
                        for (int j = 0; j < 2; j++) {
                            BigChungus chungus = new BigChungus(EntityRegistry.BIG_CHUNGUS.get(), this.getWorld());
