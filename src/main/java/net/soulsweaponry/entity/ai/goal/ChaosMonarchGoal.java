...
public class ChaosMonarchGoal extends Goal {
                    ...
                    case TELEPORT -> {
                        ...
                        if (this.attackStatus % 2 == 0 && this.attackStatus < 10) {
                            if (!this.boss.getWorld().isClient)
                                ParticleNetworking.sendServerParticlePacket((ServerWorld) this.boss.getWorld(), PacketRegistry.BIG_TELEPORT_ID, this.boss.getBlockPos(), 1000);
                        }
                        if (this.attackStatus == 23) {
                            ...
                            if (!bl) {
                                ...
                                if (!this.boss.getWorld().isClient)
                                    ParticleNetworking.sendServerParticlePacket((ServerWorld) this.boss.getWorld(), PacketRegistry.DRAGON_BREATH_EXPLOSION_ID, this.boss.getBlockPos(), 1000);
                            }
                        }
    ...
    private void hitBox(BlockPos blockPos, HitboxType type) {
        switch (type) {
            ...
            case BONK -> {
                ...
                if (!this.boss.getWorld().isClient)
                    ParticleNetworking.sendServerParticlePacket((ServerWorld) this.boss.getWorld(), PacketRegistry.DARK_EXPLOSION_ID, blockPos, 100);
            }
        }
    }
    ...
    private void explode() {
        ...
        if (!this.boss.getWorld().isClient) ParticleNetworking.sendServerParticlePacket((ServerWorld) this.boss.getWorld(), PacketRegistry.DEATH_EXPLOSION_PACKET_ID, this.boss.getBlockPos(), true);
    }
