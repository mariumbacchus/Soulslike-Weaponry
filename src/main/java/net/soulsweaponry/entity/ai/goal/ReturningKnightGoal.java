...
public class ReturningKnightGoal extends Goal {
    ...
    public void tick() {
        ...
        if (target != null && !this.boss.getSpawning()) {
            ...
            if (this.boss.getHealth() <= this.boss.getMaxHealth() / 2.0F && !this.hasUsedUnbreakable && this.attackCooldown > 20) {
                ...
                this.boss.getWorld().playSound(null, this.boss.getBlockPos(), SoundRegistry.NIGHTFALL_SHIELD_EVENT.get(), SoundCategory.HOSTILE, .75f, 1f);
            }
    ...
    private void summonAllies(Vec3d pos, boolean healer) {
        MobEntity entity = healer ? new DarkSorcerer(EntityRegistry.DARK_SORCERER.get(), this.boss.getWorld()) : new Remnant(EntityRegistry.REMNANT.get(), this.boss.getWorld());
