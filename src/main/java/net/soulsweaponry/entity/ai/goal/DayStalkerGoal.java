...
public class DayStalkerGoal extends MeleeAttackGoal {
    ...
    */
    ...
    public void tick() {
        ...
        if (this.boss.isState(DayStalker.States.INITIATING_PHASE_2)) {
    ...
    private void tickPartnerSwitch() {
        ...
        if (!this.boss.getWorld().isClient && (partner = this.boss.getPartner((ServerWorld) this.boss.getWorld())) != null) {
            if (timer == 0) {
                ...
                this.boss.flightTimer = (int) EntityConfig.duo_fight_time_before_switch;
    ...
    private void checkAndReset(int attackCooldown, int specialCooldown) {
        if (this.attackStatus > this.attackLength) {
            ...
            this.attackCooldown = MathHelper.floor((double)attackCooldown
                    * (this.boss.isPhaseTwo() ? EntityConfig.day_stalker_cooldown_modifier_phase_2 : EntityConfig.day_stalker_cooldown_modifier_phase_1));
    ...
    private float getModifiedDamage(float damage) {
        return damage * EntityConfig.day_stalker_damage_modifier;
    }
    ...
    private void inferno() {
        ...
        if (this.attackStatus >= 70 && this.attackStatus <= 100) {
            if (this.boss.isOnGround() && !this.hasExploded) {
                this.boss.getWorld().createExplosion(this.boss, this.boss.getX(), this.boss.getY(), this.boss.getZ(), 6f, World.ExplosionSourceType.TRIGGER);
                this.hasExploded = true;
            }
        }
        if (this.attackStatus == 125) {
            this.boss.getWorld().createExplosion(this.boss, this.boss.getX(), this.boss.getY(), this.boss.getZ(), 8f, World.ExplosionSourceType.TRIGGER);
        }
        this.checkAndReset(30, 0);
    }
