...
public class NightShadeGoal extends Goal {
    ...
    private void reset(float cooldownModifier) {
        this.attackStatus = 0;
        this.attackCooldown = (int) Math.floor(EntityConfig.frenzied_shade_cooldown * cooldownModifier);
    }
    ...
    private float getModifiedDamage(float damage) {
        return damage * EntityConfig.frenzied_shade_damage_modifier * (this.boss.isCopy() ? 0.35f : 1f);
    }
