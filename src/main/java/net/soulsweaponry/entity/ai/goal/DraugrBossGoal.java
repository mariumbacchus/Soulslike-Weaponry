...
public class DraugrBossGoal extends MeleeAttackGoal {
    ...
    public void reset(float cooldownModifier, boolean shieldUp) {
        this.attackStatus = 0;
        this.attackCooldown = (int)Math.floor((EntityConfig.old_champions_remains_attack_cooldown_ticks * cooldownModifier) /
    ...
    public boolean applyDamage(LivingEntity target, float baseDamage) {
        float modified = baseDamage * EntityConfig.old_champions_remains_damage_modifier;
        ...
        for (int frame : frames) {
            if (attackStatus == frame && this.isInMeleeRange(target)) {
                if (this.applyDamage(target, damage)) {
                    ...
                    if (applyBleed) {
                        BleedData.addBleed(target, (int) EntityConfig.old_champions_remains_bleed_applied);
    ...
    private void aoe(int maxTicks, int frame, float damage, float knockback, List<RegistryEntry<StatusEffect>> effects, double boxSize, boolean shieldUpWhenDone) {
        ...
        if (this.attackStatus >= maxTicks) {
            ...
            if (!effects.isEmpty()) {
                this.specialCooldown = (int) EntityConfig.old_champions_remains_special_cooldown_ticks;
            }
        }
    }
