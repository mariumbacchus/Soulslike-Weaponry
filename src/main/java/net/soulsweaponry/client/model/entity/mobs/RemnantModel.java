...
public class RemnantModel extends BipedEntityModel<RemnantRenderer.RemnantRenderState> {
    ...
    @Override
    public void setAngles(RemnantRenderer.RemnantRenderState state) {
        super.setAngles(state);

        if (state.attacking) {
            this.rightArm.pitch = 80f;
            if (state.mainHandEmpty) {
