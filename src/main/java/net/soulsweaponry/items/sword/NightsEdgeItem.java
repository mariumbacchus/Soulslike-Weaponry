...
public class NightsEdgeItem extends ChargeToUseItem implements IKeybindAbility {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (target.hasStatusEffect(EffectRegistry.BLIGHT.get())) {
            int amp = target.getStatusEffect(EffectRegistry.BLIGHT.get()).getAmplifier();
            target.addStatusEffect(new StatusEffectInstance(EffectRegistry.BLIGHT.get(), 60, amp + 1));
            if (amp >= 10) {
                target.addStatusEffect(new StatusEffectInstance(EffectRegistry.DECAY.get(), 80, 0));
            }
        } else {
            target.addStatusEffect(new StatusEffectInstance(EffectRegistry.BLIGHT.get(), 60, MathHelper.floor(WeaponUtil.getEnchantDamageBonus(stack)/2f)));
        }
        return super.postHit(stack, target, attacker);
    }
    ...
    public void useKeybindAbilityServer(ServerWorld world, ItemStack stack, PlayerEntity player) {
        if (!player.getItemCooldownManager().isCoolingDown(this)) {
            ...
            Vec2f radii = new Vec2f(1.5f, 1.75f); // base radius & per-ring increment
            float ringYaw = player.getYaw() + 90f;
            WeaponUtil.doConsumerOnCircle(world, ringYaw, player.getPos(), verticalSearch, ripples, radii, (position, warmup, yawDeg) ->
                            this.spawnNightsEdge(world, player, stack, position, warmup, yawDeg        // **now** in degrees already!
                            )
            );
