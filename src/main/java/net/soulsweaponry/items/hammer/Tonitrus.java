...
public class Tonitrus extends ModdedSword implements GeoItem {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (!this.isDisabled(stack) && attacker.hasStatusEffect(EffectRegistry.STORMVEIL.get())) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!user.hasStatusEffect(EffectRegistry.COOLDOWN.get())) {
            if (user.hasStatusEffect(EffectRegistry.STORMVEIL.get())) {
                ...
                user.addStatusEffect(new StatusEffectInstance(EffectRegistry.STORMVEIL.get(), (int) ConfigConstructor.tonitrus_stormveil_effect_duration, amp));
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        ...
        if (entity instanceof PlayerEntity player && world instanceof ServerWorld serverWorld && player.age % 10 == 0) {
            if (player.hasStatusEffect(EffectRegistry.STORMVEIL.get())) {
