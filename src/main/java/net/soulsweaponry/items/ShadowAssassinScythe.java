...
public class ShadowAssassinScythe extends UmbralTrespassItem {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (Screen.hasShiftDown()) {
            ...
            tooltip.add(new TranslatableText("tooltip.soulsweapons.early_combat_description_3").formatted(Formatting.DARK_GRAY, Formatting.ITALIC));
            tooltip.add(new LiteralText(String.valueOf(MathHelper.floor((float)TICKS_FOR_BONUS / 20f))).formatted(Formatting.AQUA));
            tooltip.add(new TranslatableText("tooltip.soulsweapons.umbral_trespass").formatted(Formatting.DARK_AQUA));
