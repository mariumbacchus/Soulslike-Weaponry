...
public record Aftershock(
    ...
    @Override
    public boolean onUserDamaged(DamageSource source, float amount, ItemStack stack, LivingEntity user, LivingEntity attacker) {
        World world = attacker.getWorld();
        if (user.getHealth() <= user.getMaxHealth() * this.percentHealthThreshold && !world.isClient && user instanceof PlayerEntity player && !this.isCoolingDown(player, stack)) {
    ...
    public List<Text> getTooltipAbilities(ItemStack stack) {
        ...
        MutableText health = Text.of(String.format("%.0f", this.percentHealthThreshold * 100) + "%").copy();
