...
public class DragonStaff extends ModdedSword {
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        if (user.isSneaking() && remainingUseTicks > 0) {
            ...
            if (world.isClient) {
                for (int k = 0; k < 10; k++) {
                    world.addParticle(ParticleRegistry.PURPLE_FLAME.get(), true, particleSpawn.add(user.getPos()).getX(), particleSpawn.add(user.getPos()).getY() + 1.5F, particleSpawn.add(user.getPos()).getZ(), pov.x + user.getRandom().nextDouble() - .25, pov.y + user.getRandom().nextDouble() - .5, pov.z + user.getRandom().nextDouble() - .25);
